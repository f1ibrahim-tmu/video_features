...
def make_path(output_root, video_path, output_key, ext):
    ...
    # /data/i5O/i5OData/undercover-right/videos/
    fname = f'{Path(video_path).parts[4].replace("/", "_")}_{Path(video_path).parts[6].replace("/", "_")}_{Path(video_path).stem}_{output_key}{ext}'
